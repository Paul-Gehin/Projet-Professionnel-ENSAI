...
point <- alt$Chart(donnees_finales)$mark_point()$encode(
  ...
  color=alt$Color('FEM:N', legend=alt$Legend(title="Efectifs hommes/femmes"),scale=alt$Scale(
    domain=c('Femmes','Hommes'),
    range=c('red','green'))),
  tooltip = c("Age_revolu:Q","nb_femmes:Q")
)

point2 <- alt$Chart(donnees_finales)$mark_point()$encode(
  y=alt$Y('nb_hommes:Q'),
  x=alt$X('Age_revolu:Q'),
  color=alt$Color('HOM:N',scale=alt$Scale(
    domain=c('Hommes'),
    range=c('green'))),
  tooltip = c("Age_revolu:Q","nb_hommes:Q")
)

bar <- alt$Chart(donnees_finales)$mark_bar()$encode(
  x=alt$X('Age_revolu:Q', axis=alt$Axis(title="Age"),
          bin = alt$Bin(maxbins = 20)),
  y=alt$Y('Ensemble:Q', axis=alt$Axis(title="Population totale en 2018")),
